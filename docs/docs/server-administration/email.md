...
By default, emails generated by Tulio (Notifications, Forgot password, update logs, etc.) are sent the internal mail. If you want, you can setup mail to be send over a SMTP account.

Execute the following script and follow the instructions:

```bash
bash /usr/local/tulio/install/upgrade/manual/configure-server-smtp.sh
```
...
Create: $TULIO/data/extensions/add-mail-domain.sh and $TULIO/data/extensions/v-delete-mail-domain.sh

### Proxmox mail server

See: [Github](https://github.com/contaura/tuliocp/pull/4365)
