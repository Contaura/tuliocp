...
mysql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $TULIO/conf/mysql.conf)
	...
	if [ '0' -ne "$?" ]; then
		if [ "$notify" != 'no' ]; then
			email=$(grep CONTACT "$TULIO/data/users/$ROOT_USER/user.conf" | cut -f 2 -d \')
...
psql_connect() {
	unset PORT
	host_str=$(grep "HOST='$1'" $TULIO/conf/pgsql.conf)
	parse_object_kv_list "$host_str"
...
is_charset_valid() {
	host_str=$(grep "HOST='$host'" $TULIO/conf/$type.conf)
	parse_object_kv_list "$host_str"
...
increase_dbhost_values() {
	...
	sed -i "s/$old_dbbases/$new_dbbases/g" $TULIO/conf/$type.conf
	sed -i "s/$old_users/$new_users/g" $TULIO/conf/$type.conf
}
...
is_dbhost_new() {
	if [ -e "$TULIO/conf/$type.conf" ]; then
		check_host=$(grep "HOST='$host'" $TULIO/conf/$type.conf)
		if [ "$check_host" ]; then
