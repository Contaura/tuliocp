...
local_backup() {
	...
	if [ "$disk_usage" -ge "$BACKUP_DISK_LIMIT" ]; then
		...
		sed -i "/ $user /d" $TULIO/data/queue/backup.pipe
	...
	chown "tulioweb":"$user" $BACKUP/$user.$backup_new_date.tar
	localbackup='yes'
...
ftp_backup() {
	# Checking config
	if [ ! -e "$TULIO/conf/ftp.backup.conf" ]; then
	...
	# Parse config
	source_conf "$TULIO/conf/ftp.backup.conf"

	# Set default port
	if [ -z "$(grep 'PORT=' $TULIO/conf/ftp.backup.conf)" ]; then
		PORT='21'
	fi
...
sftp_download() {
	source_conf "$TULIO/conf/sftp.backup.conf"
	if [ -z "$PORT" ]; then
		PORT='22'
	fi
	cd $BACKUP
...
sftp_backup() {
	# Checking config
	if [ ! -e "$TULIO/conf/sftp.backup.conf" ]; then
	...
	# Set default port
	if [ -z "$(grep 'PORT=' $TULIO/conf/sftp.backup.conf)" ]; then
		PORT='22'
	fi
...
b2_backup() {
	...
	source_conf "$TULIO/conf/b2.backup.conf"
...
rclone_backup() {
	...
	source_conf "$TULIO/conf/rclone.backup.conf"
