...
- Backups change owner files to tulioweb (#4779)
...
- Add `tulio-mail` to `tulio-users` group and create `tulio-users` group on new install #4540 #4531
...
- Update tulio-nginx to 1.27.3
- Update tulio-php to 8.3.16
...
- Add prompt to `v-update-sys-tulio-git` to install NodeJS if not present (#3779)
...
- Fixed tulio-nginx not loading with custom port
...
- Update tulio-php to 8.2.7
- Update tulio-nginx to 1.25.1
...
- Update tulio-php to 8.2.5
- Update tulio-nginx to 1.23.4
...
- TulioCP 1.7.2 fixes an issue with downloading certificates from Let's Encrypt due to the implementation of asynchronous requests which will go live on 10th April 2023. Please update your server before this date to ensure compatibility with Let's Encrypt.
...
- Fixed an issue with new tulio-zone sync and servers behind NAT or with multiple IPs (#3388 #3396)
...
- Synchronise $BIN and $TULIO/BIN (#2185 @Steveorevo)
...
- Set the correct conflicts for tulio-php on Ubuntu 22.04
...
- Updated tulio-nginx to 1.23.3
- Updated tulio-php to 8.2.4
...
- Fixed an issue in auto update script for TulioCP (#2967)
...
- Update tulio-nginx to 1.23.2
- Update tulio-php to 8.1.12
...
- Update tulio-nginx to 1.23.1
- Update tulio-php to 8.1.9
...
- Update tulio-php to 8.1.8
  - Update disable_functions list php.ini for tulio-php (#2746, #2741)
...
- Add hooks to tulio-nginx and tulio-php (#2440)
...
- Fixed an issue in configure-server-smtp.sh by loading /etc/tuliocp/tulio.conf (#2488)
...
- Update tulio-nginx to 1.22.0
  - Update OpenSSL to 3.0.3
  - Update zlib to 1.2.12
  - Update PCRE to 10.40
- Update tulio-php to 8.1.7
...
- Fixed an issue where MariaDB installations were broken when performing a clean install of TulioCP v1.5.9 (#2452 | 2446)
...
```bash
rm /usr/share/keyrings/tulio-keyring.gpg
mkdir /root/.gnupg/
...
- Limit access openbase dir tulio-php
- Fixed an issue where an email was send after install nginx command not found (#2328)

### Dependencies

- Update PCRE 8.84 to PCRE2 10.39 for tulio-nginx package
...
- Fixed an issue where the access port for TulioCP was not properly set on install (#2288 / #2291)
...
- Adjust /etc/apt/sources.list.d/tulio.list to include architecture to resolve issue with I386 missing in apt.tuliocp.com
...
- Synced up translations with TulioCP (IT, PL, RU, SK and ZN-CN updated)
...
- Add support for automated testing for TulioCP code with @drone
...
- Load $TULIO path during upgrade script (#1698)
...
- Fixed an issue where plugin-tulio-change-pasword did not change the port on v-change-sys-port (Rainloop) #1895
...
- Fixed an issue where updates for `tulio-php` were incorrectly being marked as out-of-date in the UI due to a change in our servicing and package versioning scheme.
...
- Administrators now have control over software update notifications through the following settings in `$TULIO/conf/tulio.conf` and through the Control Panel web interface:
...
- For new installations, MariaDB 10.5 is now the default version.
  - For existing installations, we've provided a manual post-install upgrade script. Please run `$TULIO/install/upgrade/manual/upgrade_mariadb.sh` to migrate to MariaDB 10.5).
...
- Added a manual migration script for apache2 mpm_event for existing installations/upgrades (`$TULIO/install/upgrade/manual/migrate_mpm_event.sh`).
...
- Improved the functionality of `v-update-sys-tulio-git` to allow user to specify GitHub repository and whether to build only core package or core and dependencies.
...
- Fixed potential upgrade issue when using v-update-sys-tulio-git.
