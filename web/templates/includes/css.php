...
if ($selected_theme !== "default") {
	// Load TulioCP-shipped themes (minified, updated/overwritten with updates) - ($TULIO/web/css/themes/*.min.css)
	$non_default_theme_path = $_SERVER["TULIO"] . "/web/css/themes/" . $selected_theme . ".min.css";
	if (file_exists($non_default_theme_path)) {
		echo '<link rel="stylesheet" href="/css/themes/' . $selected_theme . ".min.css?" . JS_LATEST_UPDATE . '">';
	}
		// Load custom theme files ($TULIO/web/css/themes/custom/*.css)
	else {
		$custom_theme_path = $_SERVER["TULIO"] . "/web/css/themes/custom/" . $selected_theme . ".min.css";
