...
function get_email_template($file, $language) {
	if (
		file_exists(
			$_SERVER["TULIO"] . "/data/templates/email/" . $language . "/" . $file . ".html",
		)
	) {
	...
	if (file_exists($_SERVER["TULIO"] . "/data/templates/email/" . $file . ".html")) {
		return file_get_contents($_SERVER["TULIO"] . "/data/templates/email/" . $file . ".html");
	}
	return false;
}
