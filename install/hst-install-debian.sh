...
if [ "$type" = "ubuntu" ]; then
	check_result 1 "You are running the wrong installer for Ubuntu. Please run tst-install.sh or hst-install-ubuntu.sh instead."
...
if [ -z "$withdebs" ] || [ ! -d "$withdebs" ]; then
	...
	if [ "$TULIO_INSTALL_VER" != "$release_branch_ver" ]; then
		...
		echo ""
		echo -e "\e[33mhttps://raw.githubusercontent.com/contaura/tuliocp/release/install/tst-install.sh\e[0m"
		echo ""
...
$TULIO/bin/v-change-user-shell "$username" nologin
