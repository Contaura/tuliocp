...
# TulioCP Control Panel upgrade script for target version 1.2.0
...
if [ -d /usr/share/roundcube ]; then
	cp -f $TULIO_INSTALL_DIR/roundcube/plugins/config_newmail_notifier.inc.php /etc/roundcube/plugins/newmail_notifier/config.inc.php
	cp -f $TULIO_INSTALL_DIR/roundcube/plugins/config_zipdownload.inc.php /etc/roundcube/plugins/zipdownload/config.inc.php
...
[ ! -f "touch $TULIO/data/queue/daily.pipe" ] && touch $TULIO/data/queue/daily.pipe
...
if [ -z "$FILE_MANAGER_CHECK" ]; then
	if [ ! -e "$TULIO/web/fm/configuration.php" ]; then
		...
		$TULIO/bin/v-change-sys-config-value 'FILE_MANAGER' 'true'
	fi
fi
...
if [ -e "/var/lib/phpmyadmin" ]; then
	PMA_ALIAS_CHECK=$(cat $TULIO/conf/tulio.conf | grep DB_PMA_ALIAS)
...
if [ -e "/var/lib/phppgadmin" ]; then
	PGA_ALIAS_CHECK=$(cat $TULIO/conf/tulio.conf | grep DB_PGA_ALIAS)
