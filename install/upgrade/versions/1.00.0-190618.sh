...
# TulioCP Control Panel upgrade script for target version 1.00.0-190618
...
if [ -z "$WEBMAIL_ALIAS" ]; then
	...
	$TULIO/bin/v-change-sys-config-value 'WEBMAIL_ALIAS' "webmail"
fi

# Update Apache and Nginx configuration to support new file structure
if [ -f /etc/apache2/apache.conf ]; then
	echo "[ * ] Updating Apache configuration..."
	mv /etc/apache2/apache.conf $TULIO_BACKUP/conf/
	cp -f $TULIO_INSTALL_DIR/apache2/apache.conf /etc/apache2/apache.conf
fi
if [ -f /etc/nginx/nginx.conf ]; then
	echo "[ * ] Updating NGINX configuration..."
	mv /etc/nginx/nginx.conf $TULIO_BACKUP/conf/
	cp -f $TULIO_INSTALL_DIR/nginx/nginx.conf /etc/nginx/nginx.conf
fi
...
if [ -d $TULIO/data/packages/ ]; then
	...
	cp -f $TULIO/data/packages/default.pkg $TULIO_BACKUP/packages/
fi

# Remove old Office 365 template as there is a newer version with an updated name
if [ -f $TULIO/data/templates/dns/o365.tpl ]; then
	rm -f $TULIO/data/templates/dns/o365.tpl
fi

# Back up and remove default index.html if it exists
if [ -f /var/www/html/index.html ]; then
	mv /var/www/html/index.html $TULIO_BACKUP/templates/
fi
...
cp -rf $TULIO_INSTALL_DIR/templates/web/unassigned/* /var/www/html/
cp -rf $TULIO_INSTALL_DIR/templates/web/skel/document_errors/* /var/www/document_errors/
...
for ipaddr in $($BIN/v-list-sys-ips plain | cut -f1); do
	...
	if [ "$WEB_SYSTEM" = "apache2" ]; then
		...
		cat $TULIO_INSTALL_DIR/apache2/unassigned.conf >> $web_conf
		...
		cp -f $TULIO_INSTALL_DIR/nginx/unassigned.inc $web_conf
		sed -i 's/directIP/'$ipaddr'/g' $web_conf
	fi
...
if [ "$php_versions" -gt 1 ]; then
	...
	for v in $($BIN/v-list-sys-php plain); do
		...
		cp -f $TULIO_INSTALL_DIR/php-fpm/dummy.conf /etc/php/$v/fpm/pool.d/
		v1=$(echo "$v" | sed -e 's/[.]//')
...
if [ -f /etc/dovecot/conf.d/15-mailboxes.conf ]; then
	mv /etc/dovecot/conf.d/15-mailboxes.conf $TULIO_BACKUP/conf/
fi
if [ -f /etc/dovecot/dovecot.conf ]; then
	# Update Dovecot configuration and restart Dovecot service
	mv /etc/dovecot/dovecot.conf $TULIO_BACKUP/conf/
	cp -f $TULIO_COMMON_DIR/dovecot/dovecot.conf /etc/dovecot/dovecot.conf
...
if [ -f /etc/exim4/exim4.conf.template ]; then
	...
	mv /etc/exim4/exim4.conf.template $TULIO_BACKUP/conf/
	cp -f $TULIO_INSTALL_DIR/exim/exim4.conf.template /etc/exim4/exim4.conf.template
...
if [ -z "$IMAP_SYSTEM" ]; then
	if [ -f /usr/bin/dovecot ]; then
		echo "[ * ] Adding missing IMAP_SYSTEM variable to tulio.conf..."
		echo "IMAP_SYSTEM = 'dovecot'" >> $TULIO/conf/tulio.conf
	fi
fi

# Run sftp jail once
$TULIO/bin/v-add-sys-sftp-jail
