...
# TulioCP Control Panel upgrade script for target version 1.4.0
...
if [ "$API" = "no" ]; then
	if [ -f "$TULIO/web/api/index.php" ]; then
		...
		$TULIO/bin/v-change-sys-api remove
	fi
fi

# Back up users existing configuration data to $TULIO/conf/defaults/tulio.conf
if [ ! -f "$TULIO/conf/defaults/tulio.conf" ]; then
	...
	cp -f $TULIO/conf/tulio.conf $TULIO/conf/defaults/tulio.conf
fi
...
if [ "$WEB_SYSTEM" = "nginx" ]; then
	...
	sed -i "s|TPL='drupal6'|TPL='drupal'|g" $TULIO/data/users/*/web.conf
	sed -i "s|TPL='drupal7'|TPL='drupal'|g" $TULIO/data/users/*/web.conf
	sed -i "s|TPL='drupal8'|TPL='drupal'|g" $TULIO/data/users/*/web.conf
	sed -i "s|TPL='codeigniter2'|TPL='codeigniter'|g" $TULIO/data/users/*/web.conf
	sed -i "s|TPL='codeigniter3'|TPL='codeigniter'|g" $TULIO/data/users/*/web.conf
fi

# Remove outdated nginx templates
echo "[ * ] Removing outdated nginx templates..."
rm -rf $TULIO/data/templates/web/nginx/php-fpm/drupal6.*tpl
rm -rf $TULIO/data/templates/web/nginx/php-fpm/drupal7.*tpl
rm -rf $TULIO/data/templates/web/nginx/php-fpm/drupal8.*tpl
...
if [ -d "$TULIO/web/images/webapps/" ]; then
	...
	rm -rf $TULIO/web/src/app/WebApp/Installers/LaravelSetup.php
	rm -rf $TULIO/web/src/app/WebApp/Installers/OpencartSetup.php
	rm -rf $TULIO/web/src/app/WebApp/Installers/PrestashopSetup.php
	rm -rf $TULIO/web/src/app/WebApp/Installers/SymfonySetup.php
	rm -rf $TULIO/web/src/app/WebApp/Installers/WordpressSetup.php
	rm -rf $TULIO/web/src/app/WebApp/Installers/Joomla
fi

# Update ClamAV configuration file
if [ -f "/etc/clamav/clamd.conf" ]; then
	cp -f $TULIO_INSTALL_DIR/clamav/clamd.conf /etc/clamav/
	$TULIO/bin/v-add-user-notification admin 'ClamAV config has been overwritten' 'Warning: If you have manualy changed /etc/clamav/clamd.conf and any changes you made will be lost an backup has been created in the /root/hst_backups folder with the original config. If you have not changed the config file you can ignore this message'
fi

##### COMMANDS FOR V1.5.X

# Back up default package and install latest version
if [ -d $TULIO/data/packages/ ]; then
	echo "[ * ] Migrating legacy default package for all users..."
	$TULIO/bin/v-rename-user-package default custom > /dev/null 2>&1
	echo "[ * ] Replacing default package..."
	cp -f $TULIO_INSTALL_DIR/packages/default.pkg $TULIO/data/packages/
fi
