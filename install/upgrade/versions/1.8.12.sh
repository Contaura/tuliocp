...
# TulioCP Control Panel upgrade script for target version 1.8.11
...
if [ -d "/var/lib/snappymail" ]; then
	chown tuliomail:tuliomail /var/lib/snappymail
	chown tuliomail:tuliomail /etc/snappymail
fi
...
if [ -d "$RC_LOG" ]; then
	chown -R tuliomail:www-data "$RC_LOG"
fi
