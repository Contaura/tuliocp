...
# TulioCP Control Panel upgrade script for target version unreleased
...
for version in $($TULIO/bin/v-list-sys-php plain); do
...
if [ -d /etc/roundcube ]; then
	if [ ! -f /etc/logrotate.d/roundcube ]; then
		...
		cp -f $TULIO_INSTALL_DIR/logrotate/roundcube /etc/logrotate.d/
	fi
fi
